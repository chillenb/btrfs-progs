...
int btrfs_num_copies(struct btrfs_fs_info *fs_info, u64 logical, u64 len)
{
	...
	if (!ce) {
		fprintf(stderr, "No mapping for %llX-%llX\n",
			(unsigned long long)logical,
			(unsigned long long)logical+len);
		return 1;
	}
	if (ce->start > logical || ce->start + ce->size < logical) {
		fprintf(stderr, "Invalid mapping for %llX-%llX, got "
			"%llX-%llX\n", (unsigned long long)logical,
			(unsigned long long)logical+len,
			  ...
			  struct btrfs_chunk *chunk, int slot)
{
	...
	if (ret < 0) {
		errno = -ret;
		error("failed to add chunk map start=%llX len=%llX: %d (%m)",
					 ...
					 u64 new_size)
{
	...
	btrfs_release_path(&path);
	printf("Fixed device size for devid %llu, old size: %llX new size: %llX\n",
				       ...
				       struct btrfs_device *device)
{
	...
	if (ret == 0) {
		error(
"found dev extents covering or beyond bytenr %llX, can not shrink the device without losing data",
			device->devid);
		return -EINVAL;
	}
...
int btrfs_fix_super_size(struct btrfs_fs_info *fs_info)
{
	...
	list_for_each_entry(device, dev_list, dev_list) {
		...
		if (!IS_ALIGNED(device->total_bytes, fs_info->sectorsize)) {
			error("device %llu total_bytes %llX not aligned to %u",
